...
class Config:
    ...
    train_label = os.path.join(train_root, 'hf_round2_train.txt')
    ...
    train_data = os.path.join(train_root, 'train.pth')

    # for train
    #训练的模型名称
    model_name = 'dnnnet'
    #在第几个epoch进行到下一个state,调整lr
    stage_epoch = [16,32,48]
    #训练时的batch大小
    batch_size = 64
    #label的类别数
    num_classes = 34
    ...
    #for test
    temp_dir=os.path.join(train_root,'temp')
